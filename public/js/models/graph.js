...
function Graph(countVertex, adjac, rGraph, rVertex, orient, clicked, reflex){
    // dist = new Array();
    // incid = new Array();
    // vector = new Array();
    ...
    //addition
    this.countClick = 0;
    this.firstColorVertex = "#69B8FF";
    this.secondColorVertex = "#ccc";
};


Graph.prototype.createVertex = function(canvas) {
    for(let i = 0; i < this.countVertex; i++) {
        let fi = i * 2 * Math.PI / this.countVertex;;
        let x = parseInt(this.rGraph * Math.cos(fi) + canvas.width / 2, 10);
        let y = parseInt(this.rGraph * Math.sin(fi) + canvas.height / 2, 10);
        this.vertex[i] = new Vertex(x, y, 65 + i);
    }
}

Graph.prototype.drawVector = function(ctx, x1, y1, x2, y2, color) {
    if (this.orient) {
        var r = 20;
        var x = x2 - x1;
        var y = y2 - y1;
...
Graph.prototype.drawCurve = function(canvas, x, y, r, startAngle, endAngle, anticlockwise, color) {
    let ctx = canvas.getContext('2d');
    if(y > canvas.height / 2){
        y += r;
    }else if (y == canvas.height / 2 && x > canvas.width / 2) {
