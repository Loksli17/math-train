...
modelMysql.prototype.validate = function(object){
    if(this.rules != undefined){
        ...
        for(let key in object){
            ...
            for(let i = 0; i < errors.required.length; i++){
                console.log(errors.required[i], key);
                if(key == errors.required[i]){
                    if(this.rulesMesseges.required != undefined){
                        errorsFields[key] = this.rulesMesseges.required;
                    }else{
                        errorsFields[key] = 'This field must be blanked';
                    }
                }
            }
        }
...
modelMysql.prototype.find = async function(method, conditions){
    ...
    if(conditions.like != undefined){
        ...
        for(let i = 0; i < conditions.like.length; i++){
            column = this.escape(conditions.like[i][0]);
            data = this.escape(conditions.like[i][1]);
            like += column + " LIKE '" + data + "' ";
